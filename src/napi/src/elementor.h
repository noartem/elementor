...
class NNativeElement : public Napi::ObjectWrap<NNativeElement> {
public:
  NNativeElement(const Napi::CallbackInfo &info);
  static Napi::Function GetClass(Napi::Env env);
  Napi::Value getInstance(const Napi::CallbackInfo &info);
  Napi::Value getSize(const Napi::CallbackInfo &info);
  Napi::Value paintBackground(const Napi::CallbackInfo &info);
  Napi::Value getChildren(const Napi::CallbackInfo &info);
  Napi::Value getClipBehaviour(const Napi::CallbackInfo &info);
private:
  std::shared_ptr<Element> instance;
};
